# PR: 핀볼게임 페이지 본문 콘텐츠 보강 (SEO 최적화)

## 📝 요약
핀볼게임 페이지(`/[locale]/pinball`)에 **6개 본문 섹션**을 추가하여 SEO 및 사용자 참여도를 대폭 개선했습니다.
...
- **내용**:
  - Planck.js 물리 엔진 소개
  - 커스텀 맵 에디터, 실시간 리더보드 설명
  - "예측 불가능한 재미" 강조
- **SEO 키워드**: "브라우저 핀볼게임", "물리엔진 게임"
- **효과**: 페이지 주제 명확화, 검색 노출 증대

#### 2. **게임 방법 (How to Play)** 🎮 ⭐ 핵심
- **내용**: UI 컴포넌트 중심 설명
  - 참가자 입력 (가중치 `*숫자`, 최대 8자)
  - 맵 선택 버튼 + 정렬 (가나다순/역순/생성일순)
  - 당첨자 선정 모드 (1등/꼴등/커스텀/상위 N명)
  - 속도 조절 슬라이더 (0.1~0.5)
  - 시작/리셋 버튼
  - 리더보드 & 미니맵 활용법
  - 전체화면 모드
- **SEO 키워드**: "핀볼게임 조작법", "게임 방법"
- **효과**: "핀볼게임 하는 법" 검색 노출

#### 3. **게임 화면 구성 (Visuals)** 🖼️
- **내용**:
  - 게임 캔버스, 리더보드, 미니맵, 당첨자 화면의 생생한 묘사
  - 이미지 alt 텍스트 3개 제공
...
- **내용**: YouTube 링크 3개
  - 8-bit Arcade Medley
  - Synthwave Mix - Retro Gaming Vibes
  - Chiptune Music Compilation
...
```diff
  "pinball": {
    "title": "핀볼 게임",
    "description": "물리 엔진 기반 핀볼 게임을 즐겨보세요",
+   "content": {
...
```diff
  "pinball": {
    "title": "Pinball Game",
    "description": "Play physics-based pinball game",
+   "content": {
+     // 동일 구조, 영어 번역
+   }
  }
...
### 3. `/app/[locale]/pinball/page.tsx`
```diff
      <button className="fullscreen-btn" />
+
+     {/* Content Sections */}
+     <div className="pinball-content-sections">
...
### 키워드 최적화
- **타겟 키워드**: 브라우저 핀볼게임, 물리엔진 게임, 핀볼게임 조작법
- **롱테일 키워드**: 핀볼게임 하는 법, 브라우저 게임 추천, 물리 기반 게임
...
5. **룰렛게임 페이지 동일 작업**: 동일한 콘텐츠 보강
6. **사용자 피드백 수집**: 실제 리뷰 데이터 추가
...
### 체크리스트
- [ ] i18n 키 경로 정확성 확인 (`pinball.content.*`)
...
- **예시**: page.tsx의 `<textarea id="names-input">` → "참가자 입력" 섹션
- **효과**: 코드와 문서의 일관성 유지
...
### 콘텐츠 규모
- **Before**: 0 단어
- **After**: ~2,000 단어 (한국어 기준)
- **증가율**: ∞ (무에서 유 창조)

### 파일 변경
- **수정**: 3개 (pages.json × 2, page.tsx × 1)
- **신규**: 1개 (PR_DESCRIPTION_PINBALL.md)

### i18n 키 증가
- **Before**: 10개 (`pinball.game.*` UI 텍스트)
- **After**: 50개 이상 (`pinball.content.*` 추가)
...
- [PR_DESCRIPTION.md](./PR_DESCRIPTION.md) - About 페이지 작업 (참고)

---

## 📝 커밋 메시지 (제안)
```
feat(pinball): 본문 콘텐츠 6개 섹션 추가 (SEO 최적화)
